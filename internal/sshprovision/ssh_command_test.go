...
func TestPinnedSSHSyncStreamCommand(t *testing.T) {
	t.Parallel()
	...
	want := []string{
		...
		"-p", "2200",
		"jesse@example.com",
		SSHGatewaySyncStreamCommand,
	}
	assertSSHCommandArgs(t, cmd.Args, want)
	if got := cmd.Args[len(cmd.Args)-1]; got != SSHGatewaySyncStreamCommand {
		t.Fatalf("last arg = %q, want %q", got, SSHGatewaySyncStreamCommand)
	}
}
